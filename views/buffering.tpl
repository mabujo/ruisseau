...
{block 'content'}
	...
	<div class="container">
		...
		<p>If you don't see the icon on the tray when you are running AceStream, Windows could have hidden it. Use the up arrow to the left of the tray and click Customise. Then set AceStream to always been shown in your tray.
		...
		<p>The first 3 options are self explanatory, and for our purposes we don't care about them, so make them unchecked.<p>
		<ul>
		...
		<p>Let's skip the disk cache limit option for a second, and look at download and upload limits. </p>
		...
		<h4>Disk cache</h4>
		<p>The two settings above and below the download and upload limits allow us to control the AceStream disk cache. The disk cache is where AceStream will store the live buffer when we are watching a stream.</p>
		<p>Using the Cache drive setting, we can choose which drive in our computer we want AceStream to store it's buffer to. It will store the buffer in a hidden folder at the root of the drive. This defaults to you c: drive, but if you would like to store this temporary data on another drive, use this setting to so. I'm using a separate partition for my AceStream cache, which is g: on my system.</p>
		<p>The disk cache limit allows use to set a maximum limit for how much space the cache can use. Here I'm using the size of my cache partition, but you don't need that much space, a couple of Gb should do it. You just need to make sure which ever drive you choose has enough free space to store the live buffer.</p>
		<ul>
			<li>Disk cache limit, Gb : <small>3</small></li>
			<li>Cache drive : <small>c: (or choose a cache drive)</small></li>
		</ul>
		<h4>AceStream login</h4>
		<p>On the AceStream, you can create a username and password. These settings are used if you want to pay AceStream to disable advertisements on streams. There aren't many ads at the moment - there is usually one at the bottom when you open a stream, but it is easily closed.</p>
